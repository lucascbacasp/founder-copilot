  ...
  const providers = [
    {
      id: 'google',
      name: 'Google',
      description: t.settings.integrations.googleDesc,
      icon: PROVIDER_ICONS.google,
    },
    {
      id: 'notion',
      name: 'Notion',
      description: t.settings.integrations.notionDesc,
      icon: PROVIDER_ICONS.notion,
    },
  ];
  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-zinc-800 p-5">
        <p className="text-sm text-zinc-500">{t.settings.integrations.loading}</p>
      </div>
    );
  }
              ...
              {connected && integration?.metadata && (
                <p className="text-xs text-zinc-600 mt-1">
                  {provider.id === 'notion' && (integration.metadata as { workspace_name?: string }).workspace_name
                    ...
                    : `${t.settings.integrations.connectedOn} ${new Date(integration.created_at).toLocaleDateString(locale === 'es' ? 'es-AR' : 'en-US')}`}
                </p>
              )}
            </div>
            <div>
